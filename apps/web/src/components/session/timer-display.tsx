...
export function TimerDisplay({
  ...
  return (
    <div className="flex items-center gap-2">
      {visible && (
        <div className={cn("flex items-center gap-1.5 font-mono text-lg font-semibold tabular-nums", colorClass)}>
          <Clock className="h-4 w-4" aria-hidden="true" />
          <span aria-live="polite" aria-label={`${Math.floor(remaining / 60)} minutes ${remaining % 60} seconds remaining`}>
            {formatted}
          </span>
        </div>
      )}
